...
if (!isset($data->user_id) || !isset($data->user_full_name) || !isset($data->user_email) || !isset($data->user_phone_number)) {
...
$stmt = $conn->prepare("UPDATE user SET user_full_name = ?, user_email = ?, user_phone_number = ? WHERE user_id = ?");
$stmt->bind_param("sssi", $user_full_name, $user_email, $user_phone_number, $user_id);
