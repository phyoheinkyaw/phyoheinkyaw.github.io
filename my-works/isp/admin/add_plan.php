...
// Handle POST requests for adding a plan
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$isp_id || !$plan_name || !$plan_speed || !$plan_price_per_month || !$plan_features) {
        echo json_encode(["success" => false, "message" => "All fields are required."]);
        exit;
    }

    // Insert the new plan into the database
    $query = "INSERT INTO subscription_plan (isp_id, plan_name, plan_speed, plan_price_per_month, plan_features) 
              VALUES (?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("issds", $isp_id, $plan_name, $plan_speed, $plan_price_per_month, $plan_features);
